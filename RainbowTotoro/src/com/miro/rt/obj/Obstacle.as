...
package com.miro.rt.obj
{
	...
	public class Obstacle extends CitrusSprite
	{
		...
		{
			// Animated obstacle.
			if (_typeObstacle == OBSTACLE_TYPE_4)
			{
				// If this is the first time the object is being used.
				if (obstacleAnimation == null)
				{
					obstacleAnimation = new MovieClip(ResAssets.getAtlas().getTextures("obstacle" + _typeObstacle + "_0"), 10);
			...
			else
			{
				...
				{
					obstacleImage = new Image(ResAssets.getAtlas().getTexture("obstacle" + _typeObstacle));
					_view = obstacleImage;
				}
				else
				{
					// If this object is being reused.
					obstacleImage.texture = ResAssets.getAtlas().getTexture("obstacle" + _typeObstacle);
		...
		private function createObstacleCrashArt():void
		{
			...
			{
				// If this is the first time the object is being used.
				obstacleCrashImage = new Image(ResAssets.getAtlas().getTexture(("obstacle" + _typeObstacle + "_crash")));
				_view = obstacleCrashImage;
			}
			else
			{
				// If this object is being reused.
				obstacleCrashImage.texture = ResAssets.getAtlas().getTexture("obstacle" + _typeObstacle + "_crash");
			}
		...
		private function createLookOutAnimation():void
		{
			...
			{
				// If this is the first time the object is being used.
				lookOutAnimation = new MovieClip(ResAssets.getAtlas().getTextures("watchOut_"), 10);
		...
		private function hidePreviousInstance():void
		{
			...
			if (obstacleAnimation != null && _typeObstacle != OBSTACLE_TYPE_4)
			{
